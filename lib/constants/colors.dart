import 'dart:ui';

class CustomColor {
  static const Color scaffoldBg = Color.fromRGBO(37, 39, 52, 1);
  static const Color bgLight1 = Color.fromRGBO(51, 54, 70, 1);
  static const Color bgLight2 = Color.fromRGBO(66, 70, 87, 1);
  static const Color textFieldBg = Color(0xffC8C9CE);
  static const Color hintDark = Color(0xff666874);
  static const Color yellowSecondary = Color.fromRGBO(255, 194, 92, 1);
  static const Color yellowPrimary = Color(0xffFFAF29);
  static const Color whitePrimary = Color(0xffEAEAEB);
  static const Color whiteSecondary = Color(0xffC8C9CE);
}
